...
pattern_datas["Kebutuhan Umum"] = "KENKO CUTTER| SWALLOW SJD|WAGON CONTAINER BOX|GAJAH TIKUS TRAP|BATERAI ABC|RAJAWALI TS PANJANG LABEL KUNING XL|PLASTIK WRAP|SENDAL|sendal|Plastik warp|SPONS CUCI PIRING SCOTCH-BRITE|TRASH BAG|PENSIL FABER CASTELL|PULPEN FABER CASTELL|LAMPU BOHLAM|Kartu|Tiket|Saldo|PRINTER INK|KERTAS|BUKU|PENSIL|PENGHAPUS|Pulpen|Materai|Baterai|PLASTIK|PLASTIK Besar|Kispray|Kuota|Pulsa"
...
pattern_datas["Ikan"] = "Ikan Salmon|Ikan|Meses"
...
pattern_datas["Alat Rumah"] = " PERALATAN MANDI AIR|CERMIN|KARPET|Tas|Sandal|LAMPU|KURSI|ALAT MASAK|KOP"
...
path_process_data = "~/Documents/UAS_ML/data_transactions.csv"
write.csv(
  preprocess,
  path_process_data,
  quote = FALSE,
...
rules_1 <- apriori(
  ...
  parameter = list(minlen = 1, sup = 0.02, conf = 0.1, maxtime = 10, target = "rules")
)
...
rules_2 <- apriori(
  data = data_transactions,
  parameter = list(minlen = 3, sup = 0.04, conf = 0.5, maxtime = 10, target = "rules")
)
...
rules_3 <- apriori(
  data = data_transactions,
  parameter = list(minlen = 5, sup = 0.02, conf = 0.1, maxtime = 10, target = "rules")
)
...
rules_4 <- apriori(
  data = data_transactions,
  parameter = list(minlen = 2, sup = 0.02, conf = 0.2, maxtime = 10, target = "rules")
)
inspect(sort(rules_4, by = "lift"))
plot(sort(rules_4, by = "lift"))

# Model 5
rules_5 <- apriori(
  data = data_transactions,
  parameter = list(minlen = 3, sup = 0.001, conf = 0.01, maxtime = 10, target = "rules")
)
inspect(sort(rules_5[1:20], by = "lift"))
plot(sort(rules_5[1:1000], by = "lift"))

# Model 6
rules_6 <- apriori(
  data = data_transactions,
  parameter = list(minlen = 2, sup = 0.5, conf = 0.001, maxtime = 10, target = "rules")
)
inspect(sort(rules_6, by = "lift"))
